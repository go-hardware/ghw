...
func TestBlock(t *testing.T) {
	...
	info, err := block.New(context.TODO())
...
func TestBlockMarshalUnmarshal(t *testing.T) {
	blocks, err := block.New(context.TODO())
	if err != nil {
		t.Fatalf("Expected no error creating block.Info, but got %v", err)
	}
...
func TestBlockUnmarshal(t *testing.T) {
	...
	if sda.DriveType != block.DriveTypeHDD || sda.StorageController != block.StorageControllerSCSI {
	...
	if zram0.DriveType != block.DriveTypeSSD {
