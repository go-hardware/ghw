...
func SafeYAML(p interface{}) string {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "error marshalling JSON: %s", err)
		return ""
	}

	var jsonObj interface{}
	if err := yaml.Unmarshal(b, &jsonObj); err != nil {
		fmt.Fprintf(os.Stderr, "error converting JSON to YAML: %s", err)
		return ""
	}

	yb, err := yaml.Marshal(jsonObj)
	if err != nil {
		fmt.Fprintf(os.Stderr, "error marshalling YAML: %s", err)
		return ""
	}
