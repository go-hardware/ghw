...
func (info *Info) GetDevice(ctx context.Context, address string) *Device {
	...
	fp := getDeviceModaliasPath(ctx, pciAddr)
	if fp == "" {
	...
	device.Revision = getDeviceRevision(ctx, pciAddr)
	if info.arch == topology.ArchitectureNUMA {
		device.Node = getDeviceNUMANode(ctx, pciAddr)
	}
	device.Driver = getDeviceDriver(ctx, pciAddr)
	return device
}
...
func (info *Info) getDevices(ctx context.Context) []*Device {
	...
	for _, link := range links {
		addr := link.Name()
		dev = info.GetDevice(ctx, addr)
		if dev == nil {
			ghwcontext.Warn(ctx, "failed to get device information for PCI address %s", addr)
		} else {
