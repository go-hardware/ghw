...
// createBlockDevices grabs all the block device pseudo-directories from
// /sys/block symlinks (excluding loopback devices) and inject them into our
// build filesystem with all but the circular symlink'd subsystem directories
func (s *snapshotter) createBlockDevices() error {
	...
	for _, devLink := range devLinks {
		...
		linkPath := filepath.Join(s.buildPath, "sys/block", dname)
		...
		// Make sure the link target is a relative path!  if we use absolute
		// path, the link target will be an absolute path starting with
		// buildDir, hence the snapshot will contain broken link.  Otherwise,
		// the unpacked directory will not have the same prefix of buildDir.
	...
	for _, f := range devFiles {
		...
		if fi.Mode()&os.ModeSymlink != 0 {
			...
			if strings.HasPrefix(fname, devName) {
				// We're interested in the directories that begin with the
